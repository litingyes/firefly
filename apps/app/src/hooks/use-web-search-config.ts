...
import {
  ...
  validateWebSearchProviderConfig,
} from '@/lib/web-search'
import { loadWebSearchConfig, saveWebSearchConfig } from '@/lib/web-search-store'

const SAVE_DEBOUNCE_MS = 300

export function useWebSearchConfig() {
  const [configMap, setConfigMap] = useState<WebSearchConfigMap>(createDefaultWebSearchConfigMap)
  ...
  useEffect(() => {
    ...
    void loadWebSearchConfig().then((loaded) => {
      if (cancelled) {
        return
      }
      setConfigMap(loaded)
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      setIsSaving(true)
      void saveWebSearchConfig(configMap)
        .catch(() => {
          toast.error('Could not save web search settings', {
  ...
  const readyCount = useMemo(
    () =>
      Object.entries(configMap).filter(([id, config]) => {
        const definition = getWebSearchProviderDefinition(id as WebSearchProviderId)
        return config.enabled && isWebSearchProviderConfigured(definition, config)
      }).length,
    [configMap],
  )

  const setFieldValue = useCallback((id: WebSearchProviderId, key: string, value: string) => {
  ...
  const setEnabled = useCallback((id: WebSearchProviderId, enabled: boolean) => {
    setConfigMap((current) => ({
      ...current,
      [id]: {
        ...current[id],
  ...
  const testConnection = useCallback(
    async (id: WebSearchProviderId): Promise<boolean> => {
      const definition = getWebSearchProviderDefinition(id)
      const config = configMap[id]
      const validationError = validateWebSearchProviderConfig(definition, config)
      ...
      const result = await testWebSearchConnection(id, config, fetch)
  ...
  const updateProvider = useCallback(
    (id: WebSearchProviderId, patch: Partial<WebSearchProviderConfig>) => {
      setConfigMap((current) => ({
        ...current,
        [id]: {
          ...current[id],
