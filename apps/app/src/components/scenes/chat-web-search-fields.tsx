...
interface ChatWebSearchFieldsProps {
  providerSelectId?: string
}

export function ChatWebSearchFields({
  providerSelectId = 'chat-web-search-provider',
}: ChatWebSearchFieldsProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {chatWebSearch.enabled && connectedProviders.length > 0 ? (
        <Field>
          <FieldLabel htmlFor={providerSelectId}>Search provider</FieldLabel>
          <FieldContent>
            <NativeSelect
              id={providerSelectId}
              onChange={(event) => {
